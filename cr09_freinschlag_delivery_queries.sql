...
--
-- Host: localhost:3306
-- Erstellungszeit: 11. Jul 2020 um 13:59
...
--
-- Stellvertreter-Struktur des Views `Query 1`
-- (Siehe unten für die tatsächliche Ansicht)
--
CREATE TABLE `Query 1` (
`country` varchar(2)
,`date` varchar(72)
,`employee` varchar(101)
,`item no` int
);
...
--
-- Stellvertreter-Struktur des Views `Query 2`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Stellvertreter-Struktur des Views `Query 3`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Stellvertreter-Struktur des Views `Query 4`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Stellvertreter-Struktur des Views `Query 5`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Stellvertreter-Struktur des Views `Query 6`
-- (Siehe unten für die tatsächliche Ansicht)
--
...
--
-- Struktur des Views `Query 1`
--
DROP TABLE IF EXISTS `Query 1`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 1`  AS  select `del_items`.`id` AS `item no`,date_format(`in_deliveries`.`date`,'%M %d %Y') AS `date`,concat(`employees`.`last_name`,' ',`employees`.`first_name`) AS `employee`,`recipients`.`country_code` AS `country` from (((`del_items` join `in_deliveries` on((`in_deliveries`.`id` = `del_items`.`fk_in_delivery`))) join `employees` on((`employees`.`id` = `del_items`.`fk_employee`))) join `recipients` on((`recipients`.`id` = `del_items`.`fk_recipients`))) where (`del_items`.`id` = 3) ;

-- --------------------------------------------------------

--
-- Struktur des Views `Query 2`
--
DROP TABLE IF EXISTS `Query 2`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 2`  AS  select `del_items`.`id` AS `item no` from (`del_items` join `employees` on((`employees`.`id` = `del_items`.`fk_employee`))) where (`employees`.`id` = 1) ;

-- --------------------------------------------------------

--
-- Struktur des Views `Query 3`
--
DROP TABLE IF EXISTS `Query 3`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 3`  AS  select `recipients`.`name` AS `recipient` from ((`recipients` join `del_items` on((`del_items`.`fk_recipients` = `recipients`.`id`))) join `tracking_codes` on((`tracking_codes`.`id` = `del_items`.`fk_tracking_code`))) where (`tracking_codes`.`code` = '407859603627772365104376561604') ;

-- --------------------------------------------------------

--
-- Struktur des Views `Query 4`
--
DROP TABLE IF EXISTS `Query 4`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 4`  AS  select `tariffs`.`country_code` AS `country_code`,`tariffs`.`max_length` AS `max_length`,`tariffs`.`max_width` AS `max_width`,`tariffs`.`max_height` AS `max_height`,`tariffs`.`max_weight` AS `max_weight`,`tariffs`.`price` AS `price`,`item_categories`.`name` AS `name` from (`tariffs` join `item_categories` on((`item_categories`.`id` = `tariffs`.`fk_item_category`))) ;

-- --------------------------------------------------------

--
-- Struktur des Views `Query 5`
--
DROP TABLE IF EXISTS `Query 5`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 5`  AS  select `employees`.`id` AS `id`,`employees`.`last_name` AS `last_name`,`employees`.`first_name` AS `first_name` from `employees` ;

-- --------------------------------------------------------

--
-- Struktur des Views `Query 6`
--
DROP TABLE IF EXISTS `Query 6`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Query 6`  AS  select date_format(`in_deliveries`.`date`,'%M %d %Y') AS `date` from `in_deliveries` ;
